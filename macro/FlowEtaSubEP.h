#ifndef FLOWETASUBEP_H
#define FLOWETASUBEP_H
#include <iostream>
#include <TFile.h>
#include <TMath.h>
#include <TVector3.h>
...
public:
  FlowEtaSubEP();
  virtual ~FlowEtaSubEP();
  
  // Set
  void SetMeanQVector();
  void SetMeanSinCosPsi();
  void SetFirstRun(Bool_t bl) { this->fFirstRun = bl; }
  void SetSecondRun(Bool_t bl) { this->fSecondRun = bl; }
  ...
  std::vector<TH2D*> GetCheckHistoQVector(Int_t i){ return (i<0) ? h2_QWest : h2_QEast; } // -1:West; +1:East 
  std::vector<TH1D*> GetCheckHistoPsi(Int_t i){ return (i<0) ? h_PsiWest : h_PsiEast; } // -1:West; +1:East
...
private:
  ...
  std::vector<TProfile2D> tp2_ReadSinPsiEast;
  std::vector<TProfile2D> tp2_ReadCosPsiEast;
  std::vector<TProfile2D> tp2_ReadSinPsiWest;
  std::vector<TProfile2D> tp2_ReadCosPsiWest;
  std::vector<TH2D*> h2_QEast;
  std::vector<TH2D*> h2_QWest;
  std::vector<TH1D*> h_PsiEast;
  std::vector<TH1D*> h_PsiWest;
